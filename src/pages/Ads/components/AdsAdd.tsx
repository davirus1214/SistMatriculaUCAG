...
import { addAds } from '../../../redux/reducers/adsSlice'
...
export const AdsAdd = () => {
    ...
    const handleAdd = async(evt: any)=> {
        ...
        const res2 = await addFirebaseDoc('/Avisos/1x9cYIlY1FaQcw9jZhf6/Anuncios',data)
    ...
    // }, [forms])
      ...
      return (
      <>
      <button type="button" className="btn btn-primary mb-2" data-bs-toggle="modal" data-bs-target="#add-ads">
        Crear Aviso
      </button>    
      <div className="modal fade" id="add-ads" data-bs-backdrop="static" data-bs-keyboard="false"  aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div className="modal-dialog modal-xl">
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                        <form onSubmit={handleAdd} id='form-modal-add-ads'>                                                              
                            <label className='form-label' htmlFor={`title-ads-add`}>Título</label>
                            <textarea className='form-control rounded-0 h-10' id={`title-ads-add`} name="titulo" value={forms.titulo} onChange={(evt) => handleChange(evt)}></textarea>
                            <label className='form-label' htmlFor={`subtitle-ads-add`}>Subtítulo</label>
                            <textarea className='form-control rounded-0 h-10' id={`subtitle-ads-add`} name="subtitulo"   value={forms.subtitulo} onChange={(evt) => handleChange(evt)}/>
                            <label className='form-label' htmlFor={`description-ads-add`}>Descripción</label>
                            <textarea className='form-control rounded-0 ' id={`description-ads-add`} name='descripcion' rows={5}  value={forms.descripcion} onChange={(evt) => handleChange(evt)}/>        
                            <div className="p-3 mb-3">
                                ...
                                <label className='form-label' htmlFor={`uploadImage-add-ads`}>Subir imagen</label>   
                                <input className="form-control mb-3" id={`uploadImage-add-ads`}  name='image_url' type="file"   onChange={(evt) => handleSetFile(evt)} required/>      
                    ...
                    <div className="modal-footer">
                        ...
                        <button type="submit" className="btn btn-primary"  form='form-modal-add-ads' >Guardar Cambios</button>
                    </div>
                </div>
            </div>
        </div>
      <Toast 
      id='toast-add-ads' 
      message='¡Se ha agregado con éxito el aviso!' 
